...
class Job {
  ...
  static async findAll(filters = {}) {
    ...
    if (minSalary !== undefined) {
      ...
      whereClauses.push(`jobs.salary >= $${queryValues.length}`);
    }

    if (hasEquity === true) {
      whereClauses.push(`jobs.equity > 0`);
    }

    if (title !== undefined) {
      queryValues.push(`%${title}%`);
      whereClauses.push(`jobs.title ILIKE $${queryValues.length}`);
    }

    if (whereClauses.length > 0) {
      jobsRes += " WHERE " + whereClauses.join(" AND ");
    }

    jobsRes += " ORDER BY jobs.title";
    const jobs = await db.query(jobsRes, queryValues);
    return jobs.rows;
  }
